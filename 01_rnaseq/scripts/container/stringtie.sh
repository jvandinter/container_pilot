...
# Create output dirs
cd "${outdir}"
mkdir -p "stringtie/${sample_id}/"

# Check whether script needs to run
if [[ -f "${outdir}/stringtie/${sample_id}/${sample_id}.gtf" ]]; then
  echo "`date` ${sample_id} GTF file already present"
  exit 0
fi

# Check strandedness for downstream tools
strandedness=$( awk -v sid="${sample_id}" '$1 == sid {print $2}' ${outdir}/check_strandedness/strandedness_all.txt)
...
apptainer exec -B /hpc:/hpc ${container_dir}/stringtie-2.1.5.sif stringtie ${bam} \
    ...
    -p 4 \
    -o "${outdir}/stringtie/${sample_id}/${sample_id}.gtf"

# Add stringtie sample GTF to list for downstream analysis
touch "${outdir}/stringtie/gtfmergefile.txt"
echo "${outdir}/stringtie/${sample_id}/${sample_id}.gtf" >> "${outdir}/stringtie/gtfmergefile.txt"
